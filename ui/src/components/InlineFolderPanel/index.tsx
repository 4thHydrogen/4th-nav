import { useState, useCallback } from "react";
import type { Tool, FolderViewMode } from "../../types";
import ToolItem from "../ToolItem";
import ListToolItem from "../ListToolItem";
import FolderSettingsPopup from "../FolderSettingsPopup";
import "./index.css";

interface InlineFolderPanelProps {
  folder: Tool;
  items: Tool[];
  noImageMode: boolean;
  onOpenTool: (tool: Tool) => void;
  onClose: () => void;
  onUpdateFolderSettings?: (id: number, folderViewMode: FolderViewMode, folderItemSize: number) => void;
}

const InlineFolderPanel = ({
  folder,
  items,
  noImageMode,
  onOpenTool,
  onClose,
  onUpdateFolderSettings,
}: InlineFolderPanelProps) => {
  const [showSettings, setShowSettings] = useState(false);
  const viewMode = folder.folderViewMode || "grid";
  const itemSize = folder.folderItemSize || 28;

  const handleViewModeChange = useCallback(
    (mode: FolderViewMode) => {
      onUpdateFolderSettings?.(folder.id, mode, itemSize);
    },
    [folder.id, itemSize, onUpdateFolderSettings]
  );

  const handleItemSizeChange = useCallback(
    (size: number) => {
      onUpdateFolderSettings?.(folder.id, viewMode, size);
    },
    [folder.id, viewMode, onUpdateFolderSettings]
  );

  return (
    <div
      className="inline-folder-panel"
      onClick={(e) => e.stopPropagation()}
      style={{ backgroundColor: folder.bgColor || undefined }}
    >
      <div className="inline-folder-header" style={{ position: "relative" }}>
        <span className="inline-folder-title">{folder.name}</span>
        <div className="inline-folder-header-actions">
          {onUpdateFolderSettings && (
            <button
              className="inline-folder-settings-btn"
              onClick={(e) => {
                e.stopPropagation();
                setShowSettings((prev) => !prev);
              }}
              title="文件夹设置"
            >
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                <circle cx="12" cy="12" r="3" />
                <path d="M19.4 15a1.65 1.65 0 00.33 1.82l.06.06a2 2 0 010 2.83 2 2 0 01-2.83 0l-.06-.06a1.65 1.65 0 00-1.82-.33 1.65 1.65 0 00-1 1.51V21a2 2 0 01-2 2 2 2 0 01-2-2v-.09A1.65 1.65 0 009 19.4a1.65 1.65 0 00-1.82.33l-.06.06a2 2 0 01-2.83 0 2 2 0 010-2.83l.06-.06A1.65 1.65 0 004.68 15a1.65 1.65 0 00-1.51-1H3a2 2 0 01-2-2 2 2 0 012-2h.09A1.65 1.65 0 004.6 9a1.65 1.65 0 00-.33-1.82l-.06-.06a2 2 0 010-2.83 2 2 0 012.83 0l.06.06A1.65 1.65 0 009 4.68a1.65 1.65 0 001-1.51V3a2 2 0 012-2 2 2 0 012 2v.09a1.65 1.65 0 001 1.51 1.65 1.65 0 001.82-.33l.06-.06a2 2 0 012.83 0 2 2 0 010 2.83l-.06.06A1.65 1.65 0 0019.32 9a1.65 1.65 0 001.51 1H21a2 2 0 012 2 2 2 0 01-2 2h-.09a1.65 1.65 0 00-1.51 1z" />
              </svg>
            </button>
          )}
          <button className="inline-folder-close" onClick={onClose}>
            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
              <path d="M18 6L6 18M6 6l12 12" />
            </svg>
          </button>
        </div>
        {showSettings && (
          <FolderSettingsPopup
            folderViewMode={viewMode}
            folderItemSize={itemSize}
            onViewModeChange={handleViewModeChange}
            onItemSizeChange={handleItemSizeChange}
            onClose={() => setShowSettings(false)}
          />
        )}
      </div>
      {viewMode === "list" ? (
        <div className="inline-folder-list">
          {items.map((item) => (
            <ListToolItem
              key={item.id}
              tool={item}
              itemSize={itemSize}
              onOpen={onOpenTool}
            />
          ))}
        </div>
      ) : (
        <div className="inline-folder-grid">
          {items.map((item, index) => (
            <ToolItem
              key={item.id}
              tool={item}
              index={index}
              isSearching={false}
              noImageMode={noImageMode}
              onContextMenu={() => {}}
              onClick={() => onOpenTool(item)}
            />
          ))}
        </div>
      )}
    </div>
  );
};

export default InlineFolderPanel;
