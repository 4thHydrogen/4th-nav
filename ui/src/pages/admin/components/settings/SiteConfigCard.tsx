    ...
    <Card title="修改网站配置" style={{ marginTop: 32 }}>
      <Spin spinning={loading}>
        <Form
          ...
          form={form}
        >
          <Form.Item label="无图模式" name="noImageMode" tooltip="开启后前台将不展示工具 logo 等图片">
            <Switch defaultChecked={Boolean(siteConfig?.noImageMode)} />
          </Form.Item>
          <Form.Item label="精简模式" name="compactMode" tooltip="开启后卡片只显示标题和 logo，若同时开启无图模式则只显示标题">
          ...
          <Form.Item label="图标密度" name="density" tooltip="控制桌面端图标显示密度，图标大小会自动适配">
            <Select
              options={[
