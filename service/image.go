package service

import (
	"encoding/binary"
	"fmt"
	"io"
	"net/http"
	"net/url"
	"strings"
	"time"

	"github.com/4thHydrogen/4th-nav/goscraper"
	"github.com/4thHydrogen/4th-nav/logger"
	"github.com/4thHydrogen/4th-nav/repository"
	"github.com/4thHydrogen/4th-nav/types"
	"github.com/4thHydrogen/4th-nav/utils"
)

const minIconSize = 64

func getProxyURL() string {
	return GetSetting().Proxy
}

func getIcon(rawURL string) string {
	proxy := getProxyURL()
	logger.LogInfo("getIcon: %s (proxy: %s)", rawURL, proxy)
	u, err := url.Parse(rawURL)
	if err != nil {
		logger.LogError("getIcon: parse url error: %s", err)
		return ""
	}
	s, err := goscraper.ScrapeWithProxy(u, 5, proxy)
	if err != nil {
		logger.LogError("getIcon: %s", err)
		return ""
	}
	best := s.Preview.SelectBestIcon()
	if best == "" {
		return ""
	}
	var result string
	if strings.Contains(best, "http:") || strings.Contains(best, "https:") {
		result = best
	} else {
		first := s.Preview.Link
		second := best
		if len(first) > 0 && first[len(first)-1:] != "/" {
			first = first + "/"
		}
		if len(second) > 0 && second[:1] == "/" {
			second = second[1:]
		}
		result = first + second
	}
	logger.LogInfo("getIcon: selected best icon: %s (candidates: %d)", result, len(s.Preview.Icons))
	return result
}

func LazyFetchLogo(rawURL string, id int64) {
	updateIconStatus(id, "fetching", "")
	logo := DiscoverIconPipeline(rawURL)
	if logo != "" {
		UpdateToolIcon(id, logo)
		updateIconStatus(id, "success", "")
		return
	}
	updateIconStatus(id, "failed", "all strategies failed")
	logger.LogInfo("LazyFetchLogo: all strategies failed for %s", rawURL)
}

func updateIconStatus(id int64, status string, errMsg string) {
	err := repository.UpdateIconStatus(id, status, errMsg)
	if err != nil {
		logger.LogInfo("updateIconStatus: failed for id %d: %s", id, err)
	}
}

func isSvgUrl(iconUrl string) bool {
	lower := strings.ToLower(iconUrl)
	return strings.Contains(lower, ".svg") || strings.Contains(lower, "svg+xml")
}

func checkIconQuality(iconUrl string) bool {
	req, err := http.NewRequest("GET", iconUrl, nil)
	if err != nil {
		return true
	}
	req.Header.Add("User-Agent", "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36")
	client := utils.NewProxiedHttpClient(getProxyURL(), 10*time.Second)
	resp, err := client.Do(req)
	if err != nil {
		return true
	}
	defer resp.Body.Close()

	contentType := strings.ToLower(resp.Header.Get("Content-Type"))
	if strings.Contains(contentType, "svg") {
		return true
	}

	data := make([]byte, 512)
	n, err := resp.Body.Read(data)
	if err != nil && n == 0 {
		return true
	}
	data = data[:n]

	w, h, ok := parseImageDimensions(data)
	if !ok {
		return true
	}
	logger.LogInfo("checkIconQuality: %s -> %dx%d", iconUrl, w, h)
	return w >= minIconSize && h >= minIconSize
}

func probeCommonIconPaths(rawURL string) string {
	u, err := url.Parse(rawURL)
	if err != nil {
		return ""
	}
	baseURL := fmt.Sprintf("%s://%s", u.Scheme, u.Host)
	paths := []string{
		"/apple-touch-icon.png",
		"/apple-touch-icon-precomposed.png",
		"/favicon-192x192.png",
		"/android-chrome-192x192.png",
	}
	client := utils.NewProxiedHttpClient(getProxyURL(), 5*time.Second)
	for _, path := range paths {
		probeURL := baseURL + path
		req, err := http.NewRequest("HEAD", probeURL, nil)
		if err != nil {
			continue
		}
		req.Header.Add("User-Agent", "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36")
		resp, err := client.Do(req)
		if err != nil {
			continue
		}
		resp.Body.Close()
		if resp.StatusCode == 200 {
			ct := strings.ToLower(resp.Header.Get("Content-Type"))
			if strings.Contains(ct, "image") {
				return probeURL
			}
		}
	}
	return ""
}

func fetchGoogleFavicon(rawURL string) string {
	u, err := url.Parse(rawURL)
	if err != nil {
		return ""
	}
	domain := u.Host
	if domain == "" {
		return ""
	}
	apiURL := fmt.Sprintf("https://www.google.com/s2/favicons?domain=%s&sz=256", domain)
	client := utils.NewProxiedHttpClient(getProxyURL(), 5*time.Second)
	resp, err := client.Get(apiURL)
	if err != nil {
		logger.LogInfo("fetchGoogleFavicon: request failed for %s: %s", domain, err)
		return ""
	}
	defer resp.Body.Close()
	if resp.StatusCode != 200 {
		return ""
	}
	ct := strings.ToLower(resp.Header.Get("Content-Type"))
	if !strings.Contains(ct, "image") {
		return ""
	}
	// Verify it's not a tiny fallback icon (Google returns a 1x1 for unknown domains)
	data, err := io.ReadAll(resp.Body)
	if err != nil || len(data) < 100 {
		return ""
	}
	if w, h, ok := parseImageDimensions(data); ok && (w < 16 || h < 16) {
		return ""
	}
	// Save via UpdateImg and return the URL
	UpdateImg(apiURL)
	return apiURL
}

func parseImageDimensions(data []byte) (width, height int, ok bool) {
	// PNG: bytes 16-19 = width, 20-23 = height (big endian)
	if len(data) >= 24 && string(data[1:4]) == "PNG" {
		w := int(binary.BigEndian.Uint32(data[16:20]))
		h := int(binary.BigEndian.Uint32(data[20:24]))
		return w, h, true
	}
	// JPEG: SOF marker
	if len(data) >= 11 && data[0] == 0xFF && data[1] == 0xD8 {
		return parseJpegDimensions(data)
	}
	// ICO: byte 6 = width (0 means 256), byte 7 = height (0 means 256)
	if len(data) >= 8 && data[0] == 0x00 && data[1] == 0x00 && data[2] == 0x01 && (data[3] == 0x00 || data[3] == 0x01) {
		w := int(data[6])
		h := int(data[7])
		if w == 0 {
			w = 256
		}
		if h == 0 {
			h = 256
		}
		return w, h, true
	}
	// WebP
	if len(data) >= 30 && string(data[8:12]) == "WEBP" {
		w := int(binary.LittleEndian.Uint16(data[26:28]))
		h := int(binary.LittleEndian.Uint16(data[28:30]))
		return w, h, true
	}
	return 0, 0, false
}

func parseJpegDimensions(data []byte) (width, height int, ok bool) {
	i := 2
	for i < len(data)-9 {
		if data[i] != 0xFF {
			i++
			continue
		}
		marker := data[i+1]
		if marker == 0xC0 || marker == 0xC2 {
			if i+9 <= len(data) {
				h := int(binary.BigEndian.Uint16(data[i+5 : i+7]))
				w := int(binary.BigEndian.Uint16(data[i+7 : i+9]))
				return w, h, true
			}
		}
		if i+3 <= len(data) && marker != 0xD8 && marker != 0xD9 {
			length := int(binary.BigEndian.Uint16(data[i+2:i+4])) + 2
			i += length
		} else {
			i += 2
		}
	}
	return 0, 0, false
}

func GetImgFromDB(url1 string) types.Img {
	result, has, err := repository.GetImageByURL(url1)
	utils.CheckErr(err)
	if !has {
		var nullImg string
		l := strings.Split(url1, ".")
		suffix := l[len(l)-1]
		if strings.Contains(suffix, "svg") {
			nullImg = ""
		} else {
			nullImg = ""
		}

		return types.Img{Id: 0, Url: url1, Value: nullImg}
	}

	return result
}

func UpdateImg(url1 string) {
	base64ImgValue := utils.GetImgBase64FromUrl(url1)
	if base64ImgValue == "" {
		return
	}
	err := repository.SaveImage(url1, base64ImgValue)
	utils.CheckErr(err)
}
