...
- [ ] 2.4 service/tools.go 中 LazyFetchLogo 调整：匹配到预设域名时跳过抓取
...
- [ ] 4.1 从 Simple Icons 下载 200+ 品牌的 SVG 图标文件
- [ ] 4.2 将现有 static/icons/brand/ 中的 PNG 文件替换为等效 SVG
- [ ] 4.3 统一所有品牌图标文件命名为小写品牌名（如 github.svg、bilibili.svg）
- [ ] 4.4 扩充 icon-presets.ts：添加 200+ 品牌的域名匹配规则
- [ ] 4.5 更新 static/icons/icons.json 索引文件
- [ ] 4.6 验证：添加主流品牌网站 URL 时自动匹配到高质量 SVG 图标

## 5. IconPicker 搜索增强

- [ ] 5.1 为 IconPicker 添加搜索输入框 UI
- [ ] 5.2 实现跨来源搜索逻辑：同时搜索静态图标文件名、内置 SVG 名称、已有工具图标
- [ ] 5.3 添加搜索无结果时的空状态提示
